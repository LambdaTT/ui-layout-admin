...
export default {
  ...
  methods: {
    ...
    getNotifications() {
      var $hdr = this;
      return Http.get('/api/usernotifications/v1/notification?$limit=15&$sort_by=2&$sort_direction=DESC')
        .then(function (response) {
          ...
          for (var i = 0; i < response.data.length; i++) {
            ...
            notification.dtCreated = notification.dt_created ? Utils.dateFormat(new Date(notification.dt_created), 'd/m/y h:i:s') : null;
    ...
    readNotification(notification) {
      if (notification.do_read != 'Y') {
        Http.put('/api/usernotifications/v1/notification/' + notification.ds_key, { 'do_read': 'Y' })
          .then(() => {
            this.selectedNotification = notification;
